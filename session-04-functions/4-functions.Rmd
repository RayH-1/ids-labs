...
vec # It is converting "1" to a character value, because vectors only hold on data type. 
```
...
You can read more on functions in [this section](https://r4ds.had.co.nz/functions.html#functions) of R for Data Science.
...
get_mode <- function(vector) {
  vector <- vector %>% na.omit()
  uniq_vector <- vector %>% unique()
  output <- uniq_vector[which.max(tabulate(match(vector, uniq_vector)))]
  return(output)
}
...
```{r}
zscale <- function(x){
  if (is.numeric(x)) { #If is.numeric means if the number is numeric, if it is true then execute the code below
  (x - mean(x, na.rm = T) / sd(x, na.rm = T))
  } else { #otherwise, print not a numeric input.
    return(paste("Not a numeric input!"))
  }
}

#Useful for error prevention.
zscale(c("hello", "world"))
```
...
```{r, eval=F}
# replace netural with neutral

study$emotions[study$emotions == 'netural'] <- 'neutral'

emotify <- function(data) {
  if(data == "happy") {
    ":)" 
  } else if (data == "sad") {
    ":("
...
study |>
  mutate(
    emotions = emotify(study$emotions)
  )
```

------------------------------------------------------------------------

# Iteration `r emo::ji("gear")`
...
for (i in seq_along(df)) {       # seq_along() similar to length() 
    #You can use 1:ncol to achieve the same effect.
...
for (i in seq_along(df)) {            #for each element (i), in the column in the dataframe
...
The `purrr` package provides functions that eliminate the need for many common for loops. The apply family of functions in base R (`apply()`, `lapply()`, `tapply()`, etc.) solve a similar problem, but purrr is more consistent and thus is easier to learn. The most useful function will be `map(.x, .f)`, where: - .x: is a vector, list, or data frame - .f: is a function - output: is a list
...
## The map\* family of functions `r emo::ji("family")`
...
## Map2 `r emo::ji("double_exclamation_mark")`
...
The function works exactly the same way as the map\* functions for a single input. One caveat that applies only to `map2()` is that both inputs need to have the **same length**!
...
m_sd <- function(x) {
  mean <- mean(x, na.rm = T)
  sd <- sd(x, na.rm = T)
  return(glue::glue("The mean is {round(mean, 0)} and the standard deviation is {round(sd, 0)}"))
}
...
There is, of course, much more to learn about functions in R and for those of you who want to take it further you can find some great resources here or here. For now, consider this as is the first exposure to functions (that can actually already get you pretty far). However, it is important that you apply `r emo::ji("nerd")` your new skills and practice further on your own.

One such skill is the question of how to integrate tidyverse functions into your own functions. Most dplyr verbs use tidy evaluation in some way. Tidy evaluation is a special type of non-standard evaluation (meaning the way how R interprets your written code) used throughout the tidyverse. There are two basic forms found in dplyr:
...
```{r eval = FALSE}
# please find the bug fixes in the comments in line
...
**Problem:** This creates global objects that match arguments names, which can become confusing and cause problems that become obvious when the function is called in a different environment. `r emo::ji("warning")` In case you choose this option, it is a good idea to clean your environment afterwards, or simply to remove all the new global objects using `rm()`.
...
Basically, add `browser()` into your function somewhere before you expect the expected error. **Note:** browser() works best within a clean .R script, it can shoot some troubles within .Rmd.
...
The section on functions and iteration is partly based on [*R for Data Science*](http://r4ds.had.co.nz/), section 5.2, [*Quantitative Politics with R*](http://qpolr.com/data.html/), chapter 3; as well as the [Tidyverse Session](https://github.com/uo-ec607/lectures/tree/master/05-tidyverse) and on the excellent slides by [*Malcolm Barrett*](https://github.com/malcolmbarrett/happy_scientist) in the course Data Science for Economists by Grant McDermott. The data for the exercises was inspired by [R for Epidemiology](https://www.r4epi.com/). The section on functions relies
